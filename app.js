...
(function () {
  ...
  function renderCompose() {
    ...
    view.innerHTML = '<div class="screen"><div class="pagehead"><button class="back" onclick="location.hash=\'#/comunidad\'">' + icon("back") + '</button><h1>Nueva publicación</h1></div>' +
      '<div class="field"><textarea id="cpBody" rows="4" placeholder="Comparte una idea, un logro, una pregunta… usa #hashtags y @menciones"></textarea></div>' +
  ...
  function renderProfile(uid) {
    ...
    Promise.all([Cloud.getProfileInfo(uid), Cloud.getFeed(uid)]).then(function (res) {
      ...
      b.innerHTML = (p.cover ? '<div class="p-cover" style="background-image:url(' + esc(Cloud.publicUrl(p.cover) || p.cover) + ')"></div>' : '<div class="p-cover empty"></div>') +
        ...
        (p.is_me ? '<div class="p-actions"><button class="btn ghost" id="prEdit">Editar perfil</button><button class="btn ghost" id="prSaved">' + icon("bookmark", "i-sm") + " Guardados</button></div>" : '<div class="p-actions"><button class="btn ' + (p.is_following ? "ghost" : "") + '" id="prFollow">' + (p.is_following ? "Siguiendo" : "Seguir") + '</button><button class="btn ghost" id="prMsg">' + icon("chat", "i-sm") + " Mensaje</button></div>") + "</div>" +
