...
(function () {
  ...
  function syncOnLogin(silent) {
    ...
    return chain.then(function () { return Cloud.listTricks(); }).then(function (rows) {
      ...
      // Repinta solo si la sincronización cambió algo: el re-render "por si
      // acaso" tras cada arranque hacía parpadear la vista recién pintada.
      if (isMain() && stateSig() !== sigBefore) route();
  ...
  // Huella barata del contenido visible: id+updatedAt de cada colección
  function stateSig() {
    var sig = function (arr) { return arr.map(function (x) { return x.id + ":" + (x.updatedAt || 0); }).sort().join("|"); };
    return sig(state.tricks) + "#" + sig(state.routines) + "#" + sig(state.gigs);
  }
  ...
  function tabbar(active) {
    ...
    return '<nav class="tabbar"><div class="tbbrand">' + mark() + "<span>App del Mago</span></div>" + tabs.map(function (x) {
      var badge = x.k === "com" && unreadNotif ? '<span class="tb-badge" id="tabComBadge">' + (unreadNotif > 9 ? "9+" : unreadNotif) + "</span>" : (x.k === "com" ? '<span class="tb-badge" id="tabComBadge" style="display:none"></span>' : "");
      return '<a href="' + x.h + '" class="' + (active === x.k ? "on" : "") + '">' + (active === x.k ? '<span class="tb-ind" aria-hidden="true"></span>' : "") + icon(x.ic) + badge + "<span>" + x.t + "</span></a>";
    }).join("") + "</nav>";
  }
  ...
  function renderLibrary() {
    ...
    if (state.tricks.length === 0) {
      body = '<div class="empty">' + emptyArt() + '<h3>Tu biblioteca está vacía</h3>' +
        ...
        '<button class="btn" onclick="location.hash=\'#/nuevo\'">Crear mi primer truco</button>' +
        '<button class="btn ghost" style="margin-top:10px" onclick="location.hash=\'#/incluidos\'">' + icon("wand", "i-sm") + ' Ver los trucos incluidos</button></div>';
    ...
    view.innerHTML =
      ...
      '<div class="search"><span class="mag">' + icon("search", "i-sm") + '</span><input id="q" type="search" enterkeyhint="search" placeholder="Buscar en mi biblioteca…" value="' + esc(filter.q) + '"></div>' +
      ...
      '<div class="pracbanner inc" id="incBanner">' + icon("wand", "i-sm") + "<span>Trucos incluidos · listos para actuar</span>" + icon("chev", "i-sm") + "</div>" +
    ...
    var ib = document.getElementById("incBanner"); if (ib) ib.addEventListener("click", function () { location.hash = "#/incluidos"; });
  ...
  function bindCards() {
    view.querySelectorAll(".card[data-id]").forEach(function (c) {
      c.addEventListener("click", function () {
        // La TARJETA ENTERA se convierte en la ficha (no solo la miniatura)
        c.style.viewTransitionName = "hero";
        location.hash = "#/truco/" + c.getAttribute("data-id");
      });
    });
    bindCardThumbs();
  }
  ...
  function renderDetail(id) {
    ...
    // Truco incluido: botón grande para actuarlo (y aprender su método si lo tiene).
    var incKey = meta.included, inc = incKey && INCLUDED[incKey];
    var incAct = inc
      ? ((inc.needsCloud && !cloudReady())
          ? '<p class="hint">Inicia sesión para actuar este truco incluido.</p>'
          : '<button class="btn" id="incActBtn">' + icon("play", "i-sm") + " " + esc(inc.actLabel) + "</button>" +
            (inc.methodHash ? '<button class="btn ghost" id="incMethodBtn">' + icon("book", "i-sm") + " Aprender el método</button>" : ""))
      : "";
    var colInfo =
      incAct +
      specs +
    ...
    view.innerHTML =
      '<div class="screen detail" style="view-transition-name:hero">' +
    ...
    var iAct = document.getElementById("incActBtn"); if (iAct && inc) iAct.addEventListener("click", function () { location.hash = inc.actHash; });
    var iMet = document.getElementById("incMethodBtn"); if (iMet && inc) iMet.addEventListener("click", function () { location.hash = inc.methodHash; });
  ...
  function paintPerform() {
    ...
    // En escena no se busca un botón: tocar la mitad derecha avanza y la
    // izquierda retrocede (el cuerpo entero es la zona táctil).
    document.getElementById("pBody").addEventListener("click", function (e) {
      var r0 = this.getBoundingClientRect();
      if (e.clientX - r0.left > r0.width / 2) { if (s.i < s.tricks.length - 1) { s.i++; snd("slide"); paintPerform(); } }
      else if (s.i > 0) { s.i--; snd("slide"); paintPerform(); }
    });
  ...
  function renderGigs() {
    ...
    if (!gs.length) {
      ...
      var next0 = gs.filter(function (g) { return (g.date || "") >= today; }).reverse(); // ascendente: el más cercano primero
      var past = gs.filter(function (g) { return (g.date || "") < today; });
      body = (next0.length ? '<div class="sec-label">Próximos</div><div class="rlist">' + next0.map(row).join("") + "</div>" : "") +
        (past.length ? '<div class="sec-label">Pasados</div><div class="rlist">' + past.map(row).join("") + "</div>" : "");
    }
    // Resumen del año: caché total y nº de bolos (solo si hay datos)
    var yr = String(new Date().getFullYear());
    var yearGigs = state.gigs.filter(function (g) { return (g.date || "").indexOf(yr) === 0; });
    var total = yearGigs.reduce(function (a, g) { var f = parseFloat(g.fee); return a + (isNaN(f) ? 0 : f); }, 0);
    var summary = yearGigs.length
      ? '<p class="subtitle">' + yr + ": <b>" + total.toLocaleString("es-ES") + " €</b> · " + yearGigs.length + (yearGigs.length === 1 ? " bolo" : " bolos") + "</p>"
      : '<p class="subtitle">Tu agenda de actuaciones.</p>';
    view.innerHTML = '<div class="screen"><h1 class="title">Bolos</h1>' + summary + body + "</div>";
  ...
  function refreshNotifBadge() {
    ...
    Cloud.getNotifications().then(function (d) {
      unreadNotif = d.unread || 0;
      var b = document.getElementById("notifBadge"); if (b) { b.style.display = unreadNotif ? "flex" : "none"; b.textContent = unreadNotif > 9 ? "9+" : unreadNotif; }
      var tb = document.getElementById("tabComBadge"); if (tb) { tb.style.display = unreadNotif ? "flex" : "none"; tb.textContent = unreadNotif > 9 ? "9+" : unreadNotif; }
    }).catch(function () {});
  }
  ...
  function setFxMode(m) {
    ...
    if (m === "lite") {
      // se despide en fundido, igual que llegó
      if (c) { c.className = ""; c.__off = setTimeout(function () { if (fxMode === "lite") c.remove(); }, 1200); }
    } else if (c) { clearTimeout(c.__off); c.className = "on"; } // reactivado a mitad de fundido
    else initAmbient();
  }
  ...
  function snd(kind) {
    ...
    try {
      ...
      if (kind === "tap") {
        ...
        var rate = 0.94 + Math.random() * 0.18;
        var src = a.createBufferSource(); src.buffer = makeCardBuf(a); src.playbackRate.value = rate;
        var g0 = a.createGain(); g0.gain.value = 0.5;
        src.connect(g0); g0.connect(dry);
        var rg0 = a.createGain(); rg0.gain.value = 0.5; g0.connect(rg0); rg0.connect(rev);
        src.start(t);
        partial(120, { at: 0.15 / rate, dur: 0.05, vol: 0.1, glide: 0.8, rev: 0.2 }); // se posa
      } else if (kind === "pluck") {
  ...
  function initSoundTouch() {
    document.addEventListener("pointerdown", function (e) {
      if (!sndEnabled || !e.target || !e.target.closest) return;
      if (e.target.closest(".btn, .chip, .tabbar a, .iconbtn, .fab, .seg button")) snd("tap");
    }, { passive: true });
    addEventListener("hashchange", function () { snd("slide"); });
  }

  /* == GIROSCOPIO: el oro reacciona al inclinar el dispositivo (foil real) == */
  function initTilt() {
    ...
    function loop() {
      ...
      r.setProperty("--gx", gx.toFixed(3)); r.setProperty("--gy", gy.toFixed(3));
      window.__tiltG = [gx, gy];
    ...
    function bind() { addEventListener("deviceorientation", onOri, { passive: true }); }
    ...
    if (typeof DOE.requestPermission === "function") {
      ...
      document.addEventListener("pointerdown", function req() {
        ...
        DOE.requestPermission().then(function (st) { if (st === "granted") bind(); }).catch(function () {});
      }, { passive: true });
    } else bind();
  }
  ...
  function initAmbient() {
    try {
      ...
      // El lienzo se prepara y pinta ANTES de entrar al DOM: si se insertara
      // vacío sería un fotograma negro opaco (parpadeo visible en tema claro).
      ...
      var FS = "precision mediump float;uniform vec2 R;uniform float T;uniform vec2 G;uniform float L;" +
        ...
        // Ambos temas calcan el --grad-page del CSS como base: el lienzo solo
        // AÑADE humo sobre un fondo identico al de la pagina, asi su aparición
        // (con fundido de opacidad) no produce ningún salto ni parpadeo.
        "float d=length(vec2((u.x-.5)/1.3,(1.-u.y+.12)/.9));" +
        // oscuro: humo dorado sobre el degradado nocturno
        "vec3 bd=mix(vec3(.098,.082,.129),vec3(.047,.043,.067),clamp(d/.46,0.,1.));" +
        "bd=mix(bd,vec3(.027,.024,.035),clamp((d-.46)/.54,0.,1.));" +
        "vec3 cd=bd+gold*s*.14*(1.1-u.y*.55)+gold*tw*.6*m;" +
        // claro: seda sobre papel de galería — veladura que sombrea y
        // abrillanta como tela, y motas de oro redondas y escasas
        "vec3 base=mix(vec3(.988,.976,.949),vec3(.945,.929,.894),clamp(d/.48,0.,1.));" +
        "base=mix(base,vec3(.902,.882,.831),clamp((d-.48)/.52,0.,1.));" +
        "float shn=smoothstep(.55,1.,s);" +
        "vec3 cl=base-vec3(.062,.053,.031)*s*(1.05-u.y*.3)+vec3(.05,.044,.026)*shn;" +
        "vec2 f2=fract(g)-.5;float rd=smoothstep(.5,.12,length(f2));" +
        "float tw2=step(.998,sp)*pow(.5+.5*sin(T*(1.+sp*2.)+sp*44.),6.)*rd;" +
        "cl=mix(cl,vec3(.72,.55,.22),tw2*.5*m);" +
      ...
      // Primer fotograma síncrono y recién entonces al DOM, con fundido de
      // opacidad: el humo "respira" al aparecer en vez de dar un golpe.
      gl.uniform1f(uT, 0); gl.uniform1f(uL, dark() ? 0 : 1); gl.uniform2f(uG, 0, 0);
      gl.drawArrays(gl.TRIANGLES, 0, 3);
      document.body.insertBefore(cv, document.body.firstChild);
      requestAnimationFrame(function () { requestAnimationFrame(function () { cv.className = "on"; }); });
      function frame(ts) {
  ...
  function updateHFades() {
    ...
    for (var i = 0; i < els.length; i++) {
      ...
      el.classList.toggle("hfade-l", el.scrollLeft > 4);
      el.classList.toggle("hfade-r", max > 4 && el.scrollLeft < max - 4);
    }
  }
  var hfR = 0;
  function queueHFades() { if (!hfR) hfR = requestAnimationFrame(function () { hfR = 0; updateHFades(); }); }
  function initHFades() {
    document.addEventListener("scroll", queueHFades, { passive: true, capture: true });
    addEventListener("resize", queueHFades);
    if (window.MutationObserver) new MutationObserver(queueHFades).observe(view, { childList: true, subtree: true });
    queueHFades(); // estado inicial del primer render
  }
    ...
    if (mode === "market") {
      Cloud.getMarket().then(function (rows) {
        ...
        function draw() {
          ...
          grid.querySelectorAll(".listcard[data-l]").forEach(function (c) { c.addEventListener("click", function () { c.style.viewTransitionName = "hero"; location.hash = "#/mercado/" + c.getAttribute("data-l"); }); });
        }
        draw();
  ...
  function renderProfile(uid) {
    ...
    Promise.all([Cloud.getProfileInfo(uid), Cloud.getFeed(uid), Cloud.getMarket(uid).catch(function () { return []; })]).then(function (res) {
      ...
      bindPostCards(b);
      b.querySelectorAll(".listcard[data-l]").forEach(function (c) { c.addEventListener("click", function () { c.style.viewTransitionName = "hero"; location.hash = "#/mercado/" + c.getAttribute("data-l"); }); });
  ...
  function renderListing(id) {
    clearTabbar();
    view.innerHTML = '<div class="screen" style="view-transition-name:hero"><div class="pagehead"><button class="back" aria-label="Volver" onclick="history.back()">' + icon("back") + '</button><h1>Truco</h1></div><div id="liBody">' + skelDetail() + '</div></div>';
    Promise.all([Cloud.getListing(id), Cloud.getReviews(id)]).then(function (res) {
      ...
      b.innerHTML = (l.cover ? '<div class="li-cover" style="background-image:url(' + cssUrl(Cloud.publicUrl(l.cover) || l.cover) + ')"></div>' : '<div class="li-cover ph">' + engraving(l.id) + mark("", true) + "</div>") +
  ...
  /* ========================= TRUCOS INCLUIDOS ======================== */
  // Catálogo de trucos incluidos (efectos listos para actuar). Cada uno se puede
  // ACTUAR y también AÑADIR A LA BIBLIOTECA del mago (aparece con sus trucos).
  var INCLUDED = {
    "lector": {
      title: "Lector Mental", cat: "Mentalismo",
      desc: "La app “lee la mente” del espectador en su propio teléfono y revela su carta o palabra.",
      actHash: "#/lector", actLabel: "Actuar", methodHash: "#/lector-metodo", needsCloud: false
    },
    "revelacion-musical": {
  ...
  function includedInLib(key) {
    for (var i = 0; i < state.tricks.length; i++) {
      var t = state.tricks[i]; if (t.meta && t.meta.included === key) return t;
    }
    return null;
  }
  // Crea (una sola vez) el truco incluido en la biblioteca del mago. Queda entre
  // sus trucos y, al abrirlo, tiene el botón para actuarlo.
  function addIncludedTrick(key) {
    var inc = INCLUDED[key]; if (!inc) return;
    var ex = includedInLib(key);
    if (ex) { location.hash = "#/truco/" + ex.id; return; }
    var now = Date.now();
    var t = {
      id: uid(), title: inc.title, category: inc.cat, difficulty: "medio", status: "dominado",
      notes: inc.desc + "\n\nTruco incluido de la app. Ábrelo para actuarlo.",
      tags: ["incluido"], media: [], photos: [], favorite: false,
      meta: { included: key }, practice: {}, createdAt: now, updatedAt: now, remote: false
    };
    state.tricks.push(t); save(); syncTrick(t);
    toast("Añadido a tu biblioteca");
    location.hash = "#/truco/" + t.id;
  }
  function renderIncluded() {
    mountTabbar("inc"); var f = document.getElementById("fabEl"); if (f) f.remove();
    var cards = Object.keys(INCLUDED).map(function (key) {
      var t = INCLUDED[key], inLib = includedInLib(key);
      var act = (t.needsCloud && !cloudReady())
        ? '<p class="hint">Este truco necesita tu cuenta en la nube. Inicia sesión para usarlo.</p>'
        : '<button class="btn" data-act="' + key + '">' + icon("play", "i-sm") + " " + esc(t.actLabel) + "</button>";
      var method = t.methodHash ? '<button class="btn ghost" data-method="' + key + '">' + icon("book", "i-sm") + " Aprender el método</button>" : "";
      var lib = inLib
        ? '<button class="btn ghost" data-open="' + inLib.id + '">' + icon("check", "i-sm") + " En tu biblioteca · abrir</button>"
        : '<button class="btn ghost" data-add="' + key + '">' + icon("plus", "i-sm") + " Añadir a mi biblioteca</button>";
      return '<div class="hero" style="margin-top:18px"><h2>' + esc(t.title) + "</h2><p>" + esc(t.desc) + "</p></div>" + act + method + lib;
    }).join("");
    view.innerHTML =
      '<div class="screen">' +
      '<h1 class="title">Trucos incluidos</h1>' +
      '<p class="subtitle">Efectos listos para actuar, con su método. Añádelos a tu biblioteca para tenerlos junto a tus trucos.</p>' +
      cards + "</div>";
    view.querySelectorAll("[data-act]").forEach(function (b) { b.addEventListener("click", function () { location.hash = INCLUDED[b.getAttribute("data-act")].actHash; }); });
  ...
  function renderPalmLock() {
    ...
    if (wpBtn && wpFile) {
      ...
      wpFile.addEventListener("change", function () {
        var fl = wpFile.files && wpFile.files[0]; if (!fl) return;
  ...
  function mrFillLinks() {
    ...
    // Solo se muestra el enlace corto /m/<código> (lo más limpio y memorizable).
    // El respaldo /r?c= sigue funcionando (mrSpectatorUrlSafe), pero no se enseña.
  }
  ...
  function mrDefaultStart() {
    if (mrState.startMode === "custom") { var n = parseInt(mrState.custom, 10); return isNaN(n) ? 0 : Math.max(0, n); }
  ...
  function renderMusicReveal() {
    ...
    if (mrState.ready) return mrRenderSession();
    var saved = "";
    ...
    view.innerHTML =
      ...
      '<div class="mr-code">/m/<b id="mrCodeBig">…</b></div>' +
  ...
  function mrRenderSession() {
    mrState.diag = { sentAt: 0, saved: false, spectator: false, sentRev: 0, deliveredRev: 0, err: "" };
    mrState.canSend = false; mrState.sending = false; mrState.sentOnce = false; mrState.awaitRev = 0;
    view.innerHTML =
      '<div class="screen">' +
      ...
      '<p class="hint" id="mrSendHint">El botón se activa cuando el espectador abre tu enlace; sin nadie conectado no se envía. Al enviar, su móvil salta <b>solo y al instante</b> a YouTube con la canción — <b>sin ningún toque</b> entre dejarlo boca abajo y subir el volumen.</p></div>' +
    ...
    document.getElementById("mrReveal").addEventListener("click", function () {
      ...
      mrState.sending = true; mrSyncSendBtn();
      mrState.diag.sentAt = Date.now(); mrState.diag.err = ""; mrRenderDiag();
      mrSetStatus(icon("refresh", "i-sm"), "Enviando la revelación…");
      mrSendWithRetry(raw, start, 0);
    });
    mrMonitor();
  ...
  // El botón de enviar solo se activa cuando hay un espectador mirando el enlace.
  // Así una revelación nunca se manda "al vacío": si no hay móvil conectado, no se envía.
  function mrSyncSendBtn() {
    var btn = document.getElementById("mrReveal"); if (!btn) return;
    if (mrState.sending) { btn.disabled = true; btn.innerHTML = icon("refresh", "i-sm") + " Enviando…"; return; }
    if (mrState.canSend) {
      btn.disabled = false;
      btn.innerHTML = icon("play", "i-sm") + " " + (mrState.sentOnce ? "Enviar de nuevo" : "Enviar revelación");
    } else {
      btn.disabled = true;
      btn.innerHTML = icon("refresh", "i-sm") + " Esperando al espectador…";
    }
  }
  // Envío fiable: el servidor valida y construye la URL (el cliente nunca la controla),
  // rechaza el envío si no hay espectador, y ante un fallo de red se reintenta solo.
  function mrSendWithRetry(raw, start, attempt) {
    Cloud.mrSendRevealLive(raw, start).then(function (res) {
      // El servidor NO lanza en estos casos: devuelve {ok:false, reason:...}.
      if (res && res.ok) {
        mrState.sending = false; mrState.sentOnce = true; mrState.diag.saved = true;
        if (res.rev) { mrState.diag.sentRev = res.rev; mrState.awaitRev = res.rev; }
        mrRenderDiag(); mrSyncSendBtn();
        mrSetStatus(icon("check", "i-sm"), "Revelación enviada (nº " + (res.rev || "") + "). Confirmando la entrega en el móvil del espectador…");
        return;
      }
      var reason = res && res.reason ? res.reason : "invalid";
      if (reason === "no_spectator") {
        // Se marchó justo antes de enviar: no se ha mandado nada. Se reactivará solo.
        mrState.sending = false; mrState.canSend = false; mrState.diag.spectator = false;
        mrState.diag.err = "sin espectador"; mrRenderDiag(); mrSyncSendBtn();
        mrSetStatus(icon("refresh", "i-sm"), "No hay ningún espectador conectado; no se ha enviado. Se enviará en cuanto abra tu enlace.");
        toast("No hay espectador conectado");
        return;
      }
      // Entrada no válida (URL/ID de YouTube): el mago corrige y reintenta a mano.
      mrState.sending = false; mrState.diag.err = reason; mrRenderDiag(); mrSyncSendBtn();
      mrSetStatus(icon("x", "i-sm"), "El enlace o ID de YouTube no es válido.");
      toast("Enlace o ID de YouTube no válido");
    }).catch(function (e) {
      // Distinguimos un fallo de red (reintentable) de un error del servidor
      // (entrada no válida: el servidor lanza con código, no se debe reintentar).
      var code = e && (e.code || e.status);
      var msg = (e && e.message) ? String(e.message) : "";
      var network = !code && /fetch|network|failed|conexi|timeout|load|abort/i.test(msg);
      if (network && attempt < 3) {
        mrState.diag.err = "reintentando (" + (attempt + 1) + ")"; mrRenderDiag();
        setTimeout(function () { mrSendWithRetry(raw, start, attempt + 1); }, 500 * (attempt + 1));
        return;
      }
      mrState.sending = false; mrState.diag.err = msg || String(code || "error"); mrRenderDiag(); mrSyncSendBtn();
      if (network) {
        mrSetStatus(icon("x", "i-sm"), "No se pudo enviar (problema de conexión). Inténtalo de nuevo.");
  ...
  function mrMonitor() {
    teardownMusicReveal();
    // Fuente de la verdad: el servidor. Sondeo del estado en vivo cada 1 s para
    // detectar rápido la presencia del espectador y confirmar la entrega.
    mrState.poll = setInterval(function () {
      if (!Cloud.mrLiveStatus) return;
      Cloud.mrLiveStatus().then(function (st) {
        ...
        if (typeof st.rev === "number") mrState.diag.sentRev = st.rev;
        if (typeof st.delivered_rev === "number") mrState.diag.deliveredRev = st.delivered_rev;
        // Confirmación de recibo: el móvil del espectador acusa la revelación (delivered_rev).
        var delivered = mrState.awaitRev && st.delivered_rev >= mrState.awaitRev;
        if (mrState.sending) {
          // durante el envío no tocamos la línea de estado (la controla mrSendWithRetry)
        } else if (delivered) {
          mrSetStatus(icon("check", "i-sm"), "Revelación recibida por el espectador (nº " + mrState.awaitRev + "). Su móvil abre YouTube.");
        } else if (mrState.awaitRev && st.reveal_saved) {
          mrSetStatus(icon("refresh", "i-sm"), "Revelación enviada (nº " + mrState.awaitRev + "). Esperando el acuse de su móvil…");
        } else if (seen) {
          mrSetStatus(icon("check", "i-sm"), "Espectador mirando el enlace. Listo para enviar.");
        } else {
          mrSetStatus(icon("refresh", "i-sm"), "Esperando a que el espectador abra tu enlace…");
        }
        mrRenderDiag();
      }).catch(function () {});
    }, 1000);
  ...
  function mrLocale() {
    var s = "en-US";
    try { s = navigator.language || (navigator.languages && navigator.languages[0]) || "en-US"; } catch (e) {}
    var p = (s || "en-US").split("-");
    return { lang: (p[0] || "en").toLowerCase(), region: (p[p.length - 1] || "").toUpperCase() };
  }
  // "Ofrecido por Google en:" — idiomas cooficiales por PAÍS (se muestran los que
  // NO usa ya el móvil). Como en Google real, varía según país e idioma.
  var MR_CC_LANGS = {
    ES: [["es", "español"], ["ca", "català"], ["gl", "galego"], ["eu", "euskara"]],
    CH: [["de", "Deutsch"], ["fr", "Français"], ["it", "Italiano"], ["rm", "Rumantsch"]],
    BE: [["nl", "Nederlands"], ["fr", "Français"], ["de", "Deutsch"]],
    CA: [["en", "English"], ["fr", "Français"]],
    LU: [["fr", "Français"], ["de", "Deutsch"], ["lb", "Lëtzebuergesch"]],
    IE: [["en", "English"], ["ga", "Gaeilge"]],
    PY: [["es", "español"], ["gn", "avañe'ẽ"]],
    IN: [["hi", "हिन्दी"], ["en", "English"], ["bn", "বাংলা"], ["te", "తెలుగు"], ["mr", "मराठी"], ["ta", "தமிழ்"]],
    ZA: [["en", "English"], ["af", "Afrikaans"], ["zu", "isiZulu"]]
  };
  var MR_OFFERED = { es: "Ofrecido por Google en:", ca: "Oferit per Google en:", gl: "Ofrecido por Google en:", eu: "Google hemen eskaintzen da:", en: "Google offered in:", fr: "Google proposé en :", de: "Google angeboten auf:", it: "Google offerto in:", nl: "Google aangeboden in:", pt: "Oferecido pelo Google em:" };
  // Nombre del país para el pie (en el idioma del móvil cuando lo sabemos).
  var MR_CC_NAME = {
    ES: { es: "España", en: "Spain" }, CH: { de: "Schweiz", fr: "Suisse", it: "Svizzera", en: "Switzerland" },
    BE: { nl: "België", fr: "Belgique", de: "Belgien", en: "Belgium" }, CA: { fr: "Canada", en: "Canada" },
    US: { es: "Estados Unidos", en: "United States" }, MX: { es: "México", en: "Mexico" }, AR: { es: "Argentina", en: "Argentina" },
    GB: { en: "United Kingdom" }, FR: { fr: "France", en: "France" }, DE: { de: "Deutschland", en: "Germany" },
    IT: { it: "Italia", en: "Italy" }, PT: { pt: "Portugal", en: "Portugal" }, IE: { en: "Ireland" }
  };
  ...
  function mrRenderSearchScreen() {
    ...
    // "Ofrecido por Google en:" según el país; oculta el idioma que ya usa el móvil.
    var ccl = MR_CC_LANGS[loc.region] || [], oLangs = [];
    for (i = 0; i < ccl.length; i++) { if (ccl[i][0] !== loc.lang) oLangs.push(ccl[i][1]); }
    if (oLangs.length) {
      var label = MR_OFFERED[loc.lang] || MR_OFFERED.en, links = "";
      for (i = 0; i < oLangs.length; i++) links += '<a href="#" onclick="return false">' + esc(oLangs[i]) + "</a>";
      offered = '<div class="mrg-offered">' + esc(label) + " " + links + "</div>";
    }
    // Nombre del país para el pie (varía por región/idioma).
    var ccn = MR_CC_NAME[loc.region], country = ccn ? (ccn[loc.lang] || ccn.en || "") : "";
    view.innerHTML =
  ...
  function renderSpectator(code) {
    var base = location.pathname.replace(/[^/]*$/, "");
    window.location.replace(location.origin + base + "m/" + encodeURIComponent(code || ""));
  }
  ...
  function renderSettings() {
    ...
    view.innerHTML =
      ...
      '<div class="setrow"><span class="si">' + icon("wand") + '</span><div class="st"><div class="t">Efectos visuales</div><div class="d">Humo dorado y foil 3D. "Ligeros" si tu móvil va justo.</div></div></div>' +
    ...
    if (sndSw) sndSw.addEventListener("click", function () {
      ...
      try { localStorage.setItem("magic_sound", sndEnabled ? "1" : "0"); } catch (e) {}
      document.getElementById("sndSw").classList.toggle("on", sndEnabled);
      if (sndEnabled) snd("chime");
    });
  ...
  function renderAuthForm() {
    ...
    view.innerHTML =
      ...
      '<div class="field"><label>Contraseña</label><div class="passwrap"><input id="aPass" type="password" autocomplete="' + (isSignup ? "new-password" : "current-password") + '" placeholder="mínimo 6 caracteres"><button type="button" class="peek" id="aPeek" aria-label="Mostrar contraseña">' + icon("theme", "i-sm") + '</button></div></div>' +
  ...
  function renderGate() {
    ...
    var pk = document.getElementById("aPeek"), pIn = document.getElementById("aPass");
    if (pk) pk.addEventListener("click", function () {
      var show = pIn.type === "password";
      pIn.type = show ? "text" : "password";
      pk.setAttribute("aria-label", show ? "Ocultar contraseña" : "Mostrar contraseña");
      pk.classList.toggle("on", show); pIn.focus();
    });
    setTimeout(function () { var em = document.getElementById("aEmail"); if (em && !em.value) { try { em.focus(); } catch (e) {} } }, 120);
  ...
  function route() {
    try {
      ...
      if (routedOnce) document.documentElement.classList.remove("boot");
      routedOnce = true;
  ...
  function boot() {
    ...
    if (cloudReady()) {
      // El splash solo si la sesión tarda de verdad (>250ms): en una recarga
      // normal se resuelve al instante y mostrarlo era un fogonazo de logo.
      var splashT = setTimeout(function () {
      ...
      Cloud.currentUser().then(function (u) {
        ...
        Cloud.onChange(function (u2, ev) {
          ...
          if (logged()) {
            startRealtime();
            // Solo re-renderiza si de verdad se acaba de iniciar sesión: los
            // eventos de cortesía (sesión inicial, token renovado) llegaban
            // justo tras el arranque y repintaban la vista ya pintada.
      ...
      }).catch(function () { clearTimeout(splashT); session = null; route(); });
    } else {
  ...
  try { localStorage.removeItem("magic_mesa_org"); localStorage.removeItem("magic_view"); } catch (e) {}
  // El primer pintado entra quieto (sin animación de .screen/.fab): al
  // recargar, el contenido aparece de una pieza en vez de "parpadear".
  // OJO: quitar la clase reinicia las animaciones del contenido visible,
  // así que solo cae dentro de un render posterior (el contenido se
  // reemplaza en la misma pasada y no hay reinicio visible).
