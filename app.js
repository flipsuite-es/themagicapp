...
(function () {
  ...
  function mrSpectatorUrl() {
    // Enlace corto: appdelmago.com/<código>. Una reescritura de Vercel sirve r.html
    // para /<código>; r.html lee el código del primer segmento de la ruta.
    var base = location.pathname.replace(/[^/]*$/, "");
    return mrState.code ? (location.origin + base + mrState.code) : "";
  }
  ...
  function mrFillLinks() {
    ...
    var alt = document.getElementById("mrLinkAlt");
    if (alt) alt.textContent = mrState.code ? ("Si no abre, usa: " + mrSpectatorUrlSafe().replace(/^https?:\/\//, "")) : "";
  }
  ...
  function renderMusicReveal() {
    ...
    view.innerHTML =
      ...
      '<div class="mr-code">/<b id="mrCodeBig">…</b></div>' +
      '<div class="mr-linkfull" id="mrLinkShort">…</div><div class="mr-linkalt" id="mrLinkAlt"></div>' +
  ...
  function mrRenderSession() {
    ...
    view.innerHTML =
      ...
      '<div class="panel mr-linkcard"><div class="sec-label">Tu enlace fijo (no cambia nunca)</div>' +
      '<div class="mr-code">/<b id="mrCodeBig">' + esc(mrState.code || "…") + "</b></div>" +
      '<div class="mr-linkfull" id="mrLinkShort">' + esc((mrSpectatorUrl() || "").replace(/^https?:\/\//, "")) + '</div><div class="mr-linkalt" id="mrLinkAlt"></div>' +
    ...
    document.getElementById("mrReveal").addEventListener("click", function () {
      ...
      Cloud.mrSendRevealLive(raw, start).then(function (res) {
        if (res && res.rev) mrState.diag.sentRev = res.rev;
        mrState.diag.saved = true; mrRenderDiag();
        mrSetStatus(icon("check", "i-sm"), "Revelación enviada (nº " + (res && res.rev ? res.rev : "") + "). El móvil del espectador debería abrir YouTube.");
        // Repetible: el botón vuelve a estar disponible para el siguiente espectador
        btn.disabled = false; btn.innerHTML = icon("play", "i-sm") + " Enviar de nuevo";
      }).catch(function (e) {
  ...
  function mrRenderDiag() {
    ...
    b.innerHTML =
      ...
      mrDiagRow("Enlace permanente", "/" + (mrState.code || "…") + " · sin caducidad") +
      mrDiagRow("Último error", d.err || "—");
  }
  function mrMonitor() {
    teardownMusicReveal();
    // Fuente de la verdad: el servidor. Sondeo del estado en vivo cada 2 s.
    mrState.poll = setInterval(function () {
      ...
      Cloud.mrLiveStatus().then(function (st) {
        ...
        mrState.diag.spectator = !!st.spectator_seen;
        ...
        if (st.delivered_rev && st.delivered_rev >= st.rev && st.reveal_saved) {
        ...
        } else if (st.spectator_seen) {
          mrSetStatus(icon("check", "i-sm"), "Espectador mirando el enlace. Todo preparado.");
        } else {
  ...
  function renderSpectator(code) {
    var base = location.pathname.replace(/[^/]*$/, "");
    window.location.replace(location.origin + base + encodeURIComponent(code || ""));
  }
