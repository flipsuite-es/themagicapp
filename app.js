...
(function () {
  ...
  function postCardHtml(p) {
    ...
    var head = '<header class="pc-head" data-mago="' + esc(p.author) + '">' + avatarHtml(Cloud.publicUrl(p.avatar), p.name || p.handle) +
      ...
      (p.author === (myProfile && myProfile.user_id) ? '<button class="pc-more" data-del="' + esc(p.id) + '">' + icon("dots", "i-sm") + "</button>" : "") + "</header>";
    var inner;
    if (isRepost) {
      var o = p.orig;
  ...
  function bindPostCards(scope) {
    ...
    s.querySelectorAll("[data-del]").forEach(function (b) {
      b.addEventListener("click", function (e) { e.stopPropagation(); var id = b.getAttribute("data-del"); if (!confirm("¿Eliminar esta publicación?")) return; Cloud.deletePost(id).then(function () { toast("Eliminada"); var art = b.closest(".postcard"); if (art) art.remove(); }).catch(function () { toast("No se pudo eliminar"); }); });
    });
  }
  ...
  function communityHeader(mode) {
    return '<div class="appbar"><h1 class="pagetitle" style="display:block">Comunidad</h1><span class="spacer"></span>' +
  ...
  function renderCommunity(mode) {
    ...
    if (mode === "market") {
      ...
      var tags = mode === "discover" ? Cloud.trendingTags().catch(function () { return []; }) : Promise.resolve([]);
      Promise.all([Cloud.getFeed(null, mode), tags]).then(function (res) {
        var rows = res[0], tt = res[1] || [];
        ...
        if (!rows.length) {
          bodyEl.innerHTML = strip + '<div class="empty" style="padding:46px 12px"><div class="big">' + icon(mode === "following" ? "people" : "people") + "</div><h3>" + (mode === "following" ? "Aún no sigues a nadie" : "Aún no hay nada") + "</h3><p>" + (mode === "following" ? "Descubre magos y síguelos para ver aquí sus publicaciones." : "Publica lo primero y empieza la comunidad.") + '</p><button class="btn" onclick="location.hash=\'' + (mode === "following" ? "#/descubrir" : "#/publicar") + "'\">" + (mode === "following" ? "Descubrir magos" : "Crear publicación") + "</button></div>";
        ...
        bodyEl.innerHTML = strip + '<div class="feed">' + rows.map(postCardHtml).join("") + "</div>";
  ...
  function renderProfile(uid) {
    ...
    Promise.all([Cloud.getProfileInfo(uid), Cloud.getFeed(uid)]).then(function (res) {
      ...
      b.innerHTML = (p.cover ? '<div class="p-cover" style="background-image:url(' + esc(Cloud.publicUrl(p.cover) || p.cover) + ')"></div>' : '<div class="p-cover empty"></div>') +
        ...
        (p.is_me ? '<div class="p-actions"><button class="btn ghost" id="prEdit">Editar perfil</button><button class="btn ghost" id="prSaved">' + icon("bookmark", "i-sm") + " Guardados</button></div>" : '<button class="btn ' + (p.is_following ? "ghost" : "") + '" id="prFollow">' + (p.is_following ? "Siguiendo" : "Seguir") + "</button>") + "</div>" +
